...
def windowed_PCA(vcf, window_size=1000000, window_step=10000, min_variants=10):
  ...
  genotype_matrix = genotypes.to_n_alt().T                  # Convert gentoype data into allele counts

  # Initialize PC1 reslts list and window midpoints
  pc1_values = []
  windowed_midpoints = []
  chromosomes = []

# Perform windowed PCA 
print("Peforming windowed PCA...")
for start in range(0, pos[-1], window_step):
    ...
    if np.sum(mask) >= min_variants:
      ...
      coords, model = allel.pca(gwindowed_genotypes, n_components=1)
      ...
      chromosomes.exted([chrom[mask][0]] * len(coords[:, 0]))
    else:
      pc1_values.extend([np.nan] * np.sum(mask))
      window_mid = (start + end) // 2
      window_midpoints.extend([window_mid] * np.sum(mask))
      chromosomes.exted([chrom[mask][0]] * np.sum(mask))
...
if __name__ == "__main__":
  ...
  plot_windowed_pca(window_midpoints, pc1_valies, chromosomes)
